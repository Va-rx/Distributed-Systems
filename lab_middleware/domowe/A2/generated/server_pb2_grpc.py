# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
"""Client and server classes corresponding to protobuf-defined services."""
import grpc
import warnings

import server_pb2 as server__pb2

GRPC_GENERATED_VERSION = '1.63.0'
GRPC_VERSION = grpc.__version__
EXPECTED_ERROR_RELEASE = '1.65.0'
SCHEDULED_RELEASE_DATE = 'June 25, 2024'
_version_not_supported = False

try:
    from grpc._utilities import first_version_is_lower
    _version_not_supported = first_version_is_lower(GRPC_VERSION, GRPC_GENERATED_VERSION)
except ImportError:
    _version_not_supported = True

if _version_not_supported:
    warnings.warn(
        f'The grpc package installed is at version {GRPC_VERSION},'
        + f' but the generated code in server_pb2_grpc.py depends on'
        + f' grpcio>={GRPC_GENERATED_VERSION}.'
        + f' Please upgrade your grpc module to grpcio>={GRPC_GENERATED_VERSION}'
        + f' or downgrade your generated code using grpcio-tools<={GRPC_VERSION}.'
        + f' This warning will become an error in {EXPECTED_ERROR_RELEASE},'
        + f' scheduled for release on {SCHEDULED_RELEASE_DATE}.',
        RuntimeWarning
    )


class EventServiceStub(object):
    """Missing associated documentation comment in .proto file."""

    def __init__(self, channel):
        """Constructor.

        Args:
            channel: A grpc.Channel.
        """
        self.getAllEvents = channel.unary_stream(
                '/EventService/getAllEvents',
                request_serializer=server__pb2.Empty.SerializeToString,
                response_deserializer=server__pb2.Event.FromString,
                _registered_method=True)
        self.subscribeEvent = channel.unary_unary(
                '/EventService/subscribeEvent',
                request_serializer=server__pb2.EventSubscriptionRequest.SerializeToString,
                response_deserializer=server__pb2.Event.FromString,
                _registered_method=True)
        self.getSubscribedEvents = channel.unary_stream(
                '/EventService/getSubscribedEvents',
                request_serializer=server__pb2.UserId.SerializeToString,
                response_deserializer=server__pb2.Event.FromString,
                _registered_method=True)
        self.unsubscribeEvent = channel.unary_unary(
                '/EventService/unsubscribeEvent',
                request_serializer=server__pb2.EventSubscriptionRequest.SerializeToString,
                response_deserializer=server__pb2.Event.FromString,
                _registered_method=True)
        self.notifySubscribers = channel.unary_stream(
                '/EventService/notifySubscribers',
                request_serializer=server__pb2.UserId.SerializeToString,
                response_deserializer=server__pb2.Event.FromString,
                _registered_method=True)


class EventServiceServicer(object):
    """Missing associated documentation comment in .proto file."""

    def getAllEvents(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def subscribeEvent(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def getSubscribedEvents(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def unsubscribeEvent(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def notifySubscribers(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_EventServiceServicer_to_server(servicer, server):
    rpc_method_handlers = {
            'getAllEvents': grpc.unary_stream_rpc_method_handler(
                    servicer.getAllEvents,
                    request_deserializer=server__pb2.Empty.FromString,
                    response_serializer=server__pb2.Event.SerializeToString,
            ),
            'subscribeEvent': grpc.unary_unary_rpc_method_handler(
                    servicer.subscribeEvent,
                    request_deserializer=server__pb2.EventSubscriptionRequest.FromString,
                    response_serializer=server__pb2.Event.SerializeToString,
            ),
            'getSubscribedEvents': grpc.unary_stream_rpc_method_handler(
                    servicer.getSubscribedEvents,
                    request_deserializer=server__pb2.UserId.FromString,
                    response_serializer=server__pb2.Event.SerializeToString,
            ),
            'unsubscribeEvent': grpc.unary_unary_rpc_method_handler(
                    servicer.unsubscribeEvent,
                    request_deserializer=server__pb2.EventSubscriptionRequest.FromString,
                    response_serializer=server__pb2.Event.SerializeToString,
            ),
            'notifySubscribers': grpc.unary_stream_rpc_method_handler(
                    servicer.notifySubscribers,
                    request_deserializer=server__pb2.UserId.FromString,
                    response_serializer=server__pb2.Event.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'EventService', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))


 # This class is part of an EXPERIMENTAL API.
class EventService(object):
    """Missing associated documentation comment in .proto file."""

    @staticmethod
    def getAllEvents(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_stream(
            request,
            target,
            '/EventService/getAllEvents',
            server__pb2.Empty.SerializeToString,
            server__pb2.Event.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def subscribeEvent(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/EventService/subscribeEvent',
            server__pb2.EventSubscriptionRequest.SerializeToString,
            server__pb2.Event.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def getSubscribedEvents(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_stream(
            request,
            target,
            '/EventService/getSubscribedEvents',
            server__pb2.UserId.SerializeToString,
            server__pb2.Event.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def unsubscribeEvent(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/EventService/unsubscribeEvent',
            server__pb2.EventSubscriptionRequest.SerializeToString,
            server__pb2.Event.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def notifySubscribers(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_stream(
            request,
            target,
            '/EventService/notifySubscribers',
            server__pb2.UserId.SerializeToString,
            server__pb2.Event.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)
